...
function wpzurb_menu_fallback() {
	...
	// Translators 1: Link to Menus, 2: Link to Customize
  	printf( __( 'Customize your menu located under %1$s or %2$s the design.', 'wpzurb' ),
  		...
  		sprintf(  __( '<a href="%s">Customize</a>', 'wpzurb' ),
  			get_admin_url( get_current_blog_id(), 'customize.php' )
  		)
  	);
  	echo '</div>';
} // Default menu fallback when no navigation is selected




function wpzurb_active_nav_class( $classes, $item ) {
...
add_filter( 'nav_menu_css_class', 'wpzurb_active_nav_class', 10, 2 );




function wpzurb_active_list_pages_class( $input ) {
    ...
    return $output;
} // use active class of ZURB Foundation on wp_list_pages output

add_filter( 'wp_list_pages', 'wpzurb_active_list_pages_class', 10, 2 );




class wpzurb_walker extends Walker_Nav_Menu {
